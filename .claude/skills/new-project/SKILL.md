...
description: >
  ...
  the template if it doesn't exist yet). Use when starting a new video before /storyboard.
---
...
## After scaffolding
- New brand → drop assets into `brands/<brand>/refs/` and run `/brand <brand>` to build the design system.
- Existing brand → skip straight ahead.
- Paste the script into `projects/<name>/script.md`, then run `/storyboard` for that project.

This is step 0 of the pipeline; `/storyboard` → `/push-sheet` → `/generate` → `/handoff` follow.
